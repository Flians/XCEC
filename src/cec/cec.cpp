...
void cec::iccad_by_stp(vector<vector<Node *> > &layers, int left_time, int max_conflicts) {
    STPProver stp_prover;
    // stp_prover.test();
    stp_prover.init_exprs(layers[0].size());
    vector<Expr> nodes(init_id);
    ...
    for (size_t i = 1; i < layers.size(); ++i)
    {
        ...
        for (size_t j = 0; j < layer.size(); ++j)
        {
            ...
            default:
                if (inputs.size() == 0)
                {
                    cerr << "The inputs is empty! in jec.iccad_by_stp!" << endl;
                    exit(-1);
                }
                res = inputs[0];
                break;
    ...
    stp_prover.handleQuery_iccad(args, left_time, max_conflicts, fout);
    vector<Expr>().swap(nodes);
    vector<Expr>().swap(args);
}